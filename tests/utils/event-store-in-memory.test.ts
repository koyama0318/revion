...
describe('event store in memory', () => {
  describe('getEvents', () => {
    it('should return empty array when no events', async () => {
      ...
      const result = await store.getEvents({ type: 'counter', id: '1' })
    ...
    it('should return events from version', async () => {
      ...
      const result = await store.getEvents({ type: 'counter', id: '1' }, 2)
  ...
  describe('getLastEventVersion', () => {
    it('should return 0 when no events', async () => {
      const store = new EventStoreInMemory()
      const result = await store.getLastEventVersion({ type: 'counter', id: '1' })
  ...
  describe('getSnapshot', () => {
    it('should return null when no snapshot', async () => {
      const store = new EventStoreInMemory()
      const result = await store.getSnapshot({ type: 'counter', id: '1' })
